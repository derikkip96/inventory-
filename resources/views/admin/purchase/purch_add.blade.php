        ...
        <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                  ...
                  <input class="form-control auto" list="items" placeholder="Search Item" id="search">
              ...
              <div class="box-body no-padding">
                ...
                <table class="table table-bordered" id="purchaseInvoice">
                  ...
                  <tr class="tbl_header_color dynamicRows">
                    ...
                    <th width="15%" class="text-center">{{ trans('message.table.rate') }}</th>
                    <th width="15%" class="text-center">{{ trans('message.table.tax') }}(%)</th>
                    <th width="10%" class="text-center">{{ trans('message.table.tax') }}</th>
                    <th width="15%" class="text-center">{{ trans('message.table.amount') }}</th>
                    <th width="10%" class="text-center">{{ trans('message.table.action') }}</th>
                  </tr>
                  <tr class="tableInfo"><td colspan="6" align="right"><strong>{{ trans('message.invoice.sub_total') }}</strong></td><td align="left" colspan="2"><strong id="subTotal"></strong></td></tr>
                  <tr class="tableInfo"><td colspan="6" align="right"><strong>{{ trans('message.invoice.totalTax') }}</strong></td><td align="left" colspan="2"><strong id="taxTotal"></strong></td></tr>
                  <tr class="tableInfo"><td colspan="6" align="right"><strong>{{ trans('message.invoice.grand_total') }}</strong></td><td align="left" colspan="2"><input type='text' name="total" class="form-control" id = "grandTotal" readonly></td></tr>
                  </tbody>
                </table>
                </div>
                <br><br>
...
function autoload_data(){

  $.ajax({
      url: "{{URL::to('purchase/item-populate')}}",
      dataType: "json",
      type: "GET",
...
function loadData(item){
    ...
    if(e.id) {
        if(!in_array(e.id, stack))
        {
          ...
          var itemDescriptionsplit ="";
          var category_name ="";


        if(e.stock_id.indexOf("30CLPET") !== -1){
          itemDescriptionsplit="3";
          category_name="30CLPET";

        }
        if(e.stock_id.indexOf("30CLRGB") !== -1){
          itemDescriptionsplit="3";
          category_name="30CLRGB";

        }
        if(e.stock_id.indexOf("50CLPET") !== -1){
          itemDescriptionsplit="5";
          category_name="50CLPET";

        }
        if(e.stock_id.indexOf("50CLRGB") !== -1){
          itemDescriptionsplit="5";
          category_name="50CLRGB";

        }
        if(e.stock_id.indexOf("150CLPET") !== -1){
          itemDescriptionsplit="15";
          category_name="150CLPET";
        ...
        // alert();
          ...
          var new_row = '<tr class="nr" id="rowid'+e.id+'">'+
                    '<td hidden ="true">'+ e.stock_id +'<input type="hidden" name="stock_id[]" value="'+e.stock_id+'"></td>'+
                    '<td>'+ e.description.split(itemDescriptionsplit)[0] +'<input type="hidden" name="description[]" value="'+e.value+'"></td>'+
                    '<td>'+ e.description+'<input type="hidden" name="description[]" value="'+e.description+'"></td>'+
                  //  '<td>'+ category_name +'<input type="hidden" name="description[]" value="'+e.value+'"></td>'+
                    '<td><input  onchange="myFunction('+e.id+')" class="form-control text-center no_units" min="1" data-id="'+e.id+'" data-rate="'+ e.price +'" type="number" id="qty_'+e.id+'" name="item_quantity[]" value="0" data-tax ="'+e.tax_rate+'"><input type="hidden" name="item_id[]" value="'+e.id+'"></td>'+
                    '<td class="text-center"><input min="0"  type="text" class="form-control text-center unitprice" name="unit_price[]" data-id = "'+e.id+'" id="rate_id_'+e.id+'" value="'+ e.price +'" data-tax="'+e.tax_rate+'" readonly></td>'+
                    '<td class="text-center">'+taxOptionList+'</td>'+
                    '<td class="text-center taxAmount">'+taxAmount+'</td>'+
                    '<td><input class="form-control text-center amount" type="text" id="amount_'+e.id+'" value="0" name="item_price[]" data-tax-rate="'+e.tax_rate+'" readonly></td>'+
                    '<td class="text-center"><button id="'+e.id+'" class="btn btn-xs btn-danger delete_item" style="color:#00A751;!important"><i class="glyphicon glyphicon-trash"></i></button></td>'+
                    '</tr>';
          ...
          if(e.id) {
              if(!in_array(e.id, stack))
              {
                ...
                var new_row = '<tr class="nr" id="rowid'+e.id+'">'+
                          ...
                          '<td><input class="form-control text-center no_units" min="1" data-id="'+e.id+'" data-rate="'+ e.price +'" type="text" id="qty_'+e.id+'" name="item_quantity[]" value="1" data-tax ="'+e.tax_rate+'"><input type="hidden" name="item_id[]" value="'+e.id+'"></td>'+
