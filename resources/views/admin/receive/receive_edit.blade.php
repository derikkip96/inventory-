...
@section('content')
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <!-- /.box-header -->
        <div class="box-body">
        <form action="{{url('receive/update')}}" method="POST" id="receiveForm">
        <input type="hidden" value="{{csrf_token()}}" name="_token" id="token">
        <input type="hidden" value="{{$oderInfo->order_no}}" name="order_no" id="order_no">
        <input type="hidden" value="{{$oderInfo->id}}" name="id" id="id">
          ...
          <div class="col-md-8">
            ...
            <h5>
              Receive No # {{ sprintf("%04d", $oderInfo->id) }}
            </h5>
              ...
              <div class="box-body no-padding">
                ...
                <table class="table table-bordered" id="salesInvoice">
                  ...
                  ?>
                    ...
                    <?php
                      $amount = $result->unit_price*$result->quantity;
                      $subtotal += $amount;
                    ...
                    ?>
                      @if(($result->quantity)>0)
                        ...
                        <td class="text-center" id="orderqtyid{{$result->item_id}}">{{itemQtyInOrder($oderInfo->order_no,$oderInfo->id,$result->item_code)}}</td>


                          <td><input class="form-control text-center no_units" name="quantity[]" value="{{$result->quantity}}" rowid="{{$result->item_id}}" type="text">
                          <input type="hidden" name="old_qty[]" value="{{$result->quantity}}">
                          </td>
