                ...
                <table class="table table-bordered" id="salesInvoice">
                  ...
                  ?>
                    @foreach($invoiceData as $k=>$result)
                        ...
                        <?php
                        $qtyAvailable = getAvailableQtyByLocation($result->stock_id,$result->location);
                        ?>
                        <?php
                        if($qtyAvailable < $result->item_rest){
                         $qtyStatus = 1;
                        }
                        ?>

                        <tr class="<?= ($qtyAvailable < $result->item_rest) ? 'insufficient' : '' ?>" data-tax_type="{{$result->tax_rate}}" id="rowid{{$result->item_id}}">
                          ...
                          <td>
                            <input class="form-control text-center no_units" stock-id="{{$result->stock_id}}" min="1" max={{$qtyAvailable}} data-id="{{$result->item_id}}" id="qty_{{$result->item_id}}" name="item_quantity[]" value="{{$result->item_rest}}" data-tax="{{$result->tax_rate}}" type="text" required="true">
                            <input name="item_id[]" value="{{$result->item_id}}" type="hidden"></td>
