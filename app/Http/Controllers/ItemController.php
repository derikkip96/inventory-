...
class ItemController extends Controller
{
    ...
    //     $data['itemData'] = DB::table('item_code')->where(['deleted_status'=>0])->count();
    //     $data['itemQuantity'] = DB::table('stock_moves')->select(DB::raw('SUM(qty) as total_item'))->first();
    //     $itemList = $this->report->getInventoryStockOnHand('all','all');
    //     //d($itemList,1);
    //     foreach ($itemList as $key => $item) {
    ...
    //     //return view('admin.item.item_list', $data);
    //    return $dataTable->render('admin.item.view',$data);
           ...
           //d($csv,1);
            ...
            foreach ($csv as $key => $value) {
                if($key != 0) {
                    ...
                    $data[$key]['item_id'] = mb_convert_encoding(trim($value[0]), 'ISO-8859-1');
                    $data[$key]['item_name'] = mb_convert_encoding(trim($value[1]), 'ISO-8859-1');
                    $data[$key]['long_description'] = mb_convert_encoding(trim($value[2]), 'ISO-8859-1');
                    $data[$key]['item_category'] = mb_convert_encoding(trim($value[3]), 'ISO-8859-1');
                    $data[$key]['unit'] = mb_convert_encoding(trim($value[4]), 'ISO-8859-1');
                    $data[$key]['tax_type'] = mb_convert_encoding(trim($value[5]), 'ISO-8859-1');
                    $data[$key]['purch_price'] = mb_convert_encoding(trim($value[6]), 'ISO-8859-1');
                    $data[$key]['retail_price'] = mb_convert_encoding(trim($value[7]), 'ISO-8859-1');
                    $data[$key]['whole_price'] = mb_convert_encoding(trim($value[8]), 'ISO-8859-1');
                }
            }
            ...
            if (!empty($data)) {
                ...
                foreach ($item as $key => $value) {
                    ...
                    $itemCount     = DB::table('item_code')->where(['stock_id'=>$value['item_id'],'deleted_status'=>0])->count();
                    ...
                    $taxCount      = DB::table('item_tax_types')->where('name','=',$value['tax_type'])->count();
                    //d($itemCount);
                    /*d($value);
                    d($itemCount);
                    d($categoryCount);
                    ...
                    if ($itemCount == 0 && $categoryCount > 0 && $unitCount > 0 && $taxCount>0) {
                        //d($data);
                        $itemCode[] = [
                        ...
                        $purchPrice[] = [
                                ...
                                'price' =>preg_replace('/[\$,]/', '', $value['purch_price']),
                               // 'supplier_id' => 0
                            ];
                        ...
                        $salePrice[] = [
                                ...
                                'r_price' => str_replace(",", "", $value['retail_price']),
                                //'w_price' => $value['whole_price']
                                'w_price' => str_replace(",", "", $value['whole_price'])
                            ];
                    }
                }
               // d($purchPrice);
                ...
                if(!empty($salePrice)){
                    ...
                    foreach ($salePrice as $key => $value) {
                        ...
                        $retailPrice[$key]['price'] = preg_replace('/[\$,]/', '', $value['r_price']) ? preg_replace('/[\$,]/', '', $value['r_price']) : 0;
                        $retailPrice[$key]['curr_abrev'] = 'USD';

                        $wholePrice[$key]['stock_id'] = strtoupper($value['stock_id']);
                        $wholePrice[$key]['sales_type_id'] = 2;
                        $wholePrice[$key]['price'] = preg_replace('/[\$,]/', '', $value['w_price']) ? preg_replace('/[\$,]/', '', $value['w_price']) : 0;
